...
| `-j, --jobs` | int | 0 | 并行进程数，0=自动；CSV 多行按行并行，单配方/单行任务 ≥8 进程时按 2×2×2 分块并行放置 |
| `--list` | - | - | 列出所有支持的氧化物 |
...
symbols, positions, box, formula_units, atom_counts = build_glass_model(
    ...
    jobs=0,            # 并行进程数：0=自动，1=串行，>=8 且原子足够时分块并行
)
...
- **放置**：cell-list 邻域 + 每批候选位置一次性 ragged 展开批量算距
- **弛豫**：Verlet 邻居表（rc+skin 建表、任一原子位移超 skin/2 才重建）+ 截断对列表向量化；势能在 1.25·d_min 处精确为零，截断不引入任何近似
...
|------|------|
| 220 原子，带弛豫 | ~0.2 s |
| 30000 原子，带弛豫，自动并行 | ~20-25 s |
| 30000 原子，带弛豫，串行 `-j 1` | ~50 s |

> 旧版（逐对 Python 循环）220 原子需 ~64 s，30000 原子需 100 小时以上。

两种并行粒度（`-j`，0=自动）：

- **CSV 多行**：按行多子进程并行，行内串行（每个配方一个独立结构，天然可并行）。
- **单个结构**：原子数 ≥4800 且进程数 ≥8 时，自动按 2×2×2（`-j ≥ 27` 且每块 ≥600 原子时 3×3×3）立方块分块并行放置；每块独立做周期性放置（密度与全局一致），拼接后全局弛豫消除块间界面接触（并行路径弛豫步数自动加倍，残留接触深度 ≤0.01 Å，DFT/MD 弛豫第一步即可消除）。

可复现性：同 seed 结果确定可复现；与旧版本的同 seed 坐标不逐位相同（算法升级所致），结构统计质量等价。
...
**Q: 弛豫开关怎么选？**
- 需要高质量初始结构 → 开弛豫（默认），220 原子 <0.5s，30000 原子约 20s（自动并行）
- 批量初筛 / 快速迭代 → `--no-relax`，更快
